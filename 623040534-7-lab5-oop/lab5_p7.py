...
def calculator(a, b, operation=ADD, output_format=float):
    ...
    if output_format == float:
        ...
        raise ValueError("Format must be flout or int.")
    return result

def flexible_calculator(operation=ADD, output_format=float, *args):
    if len(args    ) == 0:
        print("At lease one number must be enterd")
        return None
    if operation == ADD:
        result = 0
        for i in args:
...
if __name__ == '__main__' :
    ...
    print("calcutor(2, 3) = {}".format(answer))
    answer = calculator(2, 3, MUL)
    print("calcutor(2, 3, MUL) = {}".format(answer))
    answer = calculator(2, 3, SUB, int)
    print("calcutor(2, 3, SUB, int) = {}".format(answer))
    answer = flexible_calculator(ADD, int, 2, 3)
    print("flexible_calculator(ADD, int, 2, 3) = {}".format(answer))
